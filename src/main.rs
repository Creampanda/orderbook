...
fn main() {
    ...
    let bob_wallet = Wallet::create_wallet(2, String::from("Bob"), String::from("bob_pub_key"));
    let mark_wallet = Wallet::create_wallet(3, String::from("Mark"), String::from("mark_pub_key"));
    let ann_wallet = Wallet::create_wallet(4, String::from("Ann"), String::from("ann_pub_key"));
    ...
    for (_id, user) in users.iter_mut() {
        user.deposite(CurrencyName::XTZ, 1000000);
        user.deposite(CurrencyName::USDT, 1000000);
    }

    for _i in 1..=10 {
        let user = users.get_mut(&(rng.gen_range(1..5))).unwrap();
